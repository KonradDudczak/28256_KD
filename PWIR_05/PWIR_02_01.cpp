...
int main(){
    ...
    start = std::chrono::high_resolution_clock::now();
    #pragma omp parallel for num_threads(8) shared(matrix) private(i, k)
    for(i = 0;i<MATRIX_H;i++){
        for(k = 0;k<MATRIX_W;k++){
            matrix[i][k] = (uint16_t)(rand() % 100);           
        }
    ...
    #pragma omp parallel for num_threads(8) shared(vector) private(i)
    for(i = 0;i<VECTOR_S;i++){
        vector[i] = (uint16_t)(rand() % 100);
    }
    end = std::chrono::high_resolution_clock::now();

    printf("Fill parallel way in %lu miliseconds with 8 threads\n",
    std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count());

    start = std::chrono::high_resolution_clock::now();
    for(uint32_t p = 0; p < MUL_TIME;p++){
        #pragma omp parallel for num_threads(8) shared(matrix, vector, result) private(i, k, z)
    ...
    printf("Calculated parallel way in %lu miliseconds with 8 threads\n",
    std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count());
