...
#define MATRIX_H 6000
#define MATRIX_W 6000
#define VECTOR_S 6000
#define MUL_TIME 25
...
int main(){
    ...
    #pragma omp parallel for schedule(static, MATRIX_H/10) shared(matrix, vector, result) private(i, k)
        ...
        timestatic = timestatic + std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count();
    }

    timestatic = timestatic / MUL_TIME;
    printf("Sredni czas dla static to %llu milisekund\n",timestatic);

    

    long long timedynamic = 0;
    for(uint32_t p = 0; p < MUL_TIME;p++){
        start = std::chrono::high_resolution_clock::now();
        ...
        timedynamic = timedynamic + std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count();
    }
    
    
    timedynamic = timedynamic / MUL_TIME;
    printf("Sredni czas dla dynamic to %llu milisekund\n", timedynamic);
    ...
    for(uint32_t p = 0; p < MUL_TIME;p++){
        ...
        timeguided = timeguided + std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count();

    
    }   
    
    timeguided = timeguided / MUL_TIME;
    printf("Sredni czas dla guided to %llu milisekund\n", timeguided);
    ...
    for(uint32_t p = 0; p < MUL_TIME;p++){
        ...
        timeruntime = timeruntime + std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count();

    }
    
    timeruntime = timeruntime / MUL_TIME;
    printf("Sredni czas dla runtime to %llu milisekund\n", timeruntime);
